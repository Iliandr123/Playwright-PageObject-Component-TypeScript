...
export abstract class Component {
    ...
    getLocator(locatorProps: LocatorProps = {}): Locator {
        const { locator, ...context } = locatorProps;
    ...
    get componentName(): string {
        if (!this.name) {
            throw Error('Provide component name');
        } else {
            return this.name;
        }
    }
    ...
    private getErrorMessage(action: string): string {
        return `${this.typeOf} with name ${this.componentName} and locator ${this.locator} ${action}`
    }

    async shouldBeVisible(locatorProps: LocatorProps = {}): Promise<void> {
        await test.step(`${this.typeOf} "${this.componentName}" should be visible on the page`, async () => {
    ...
    async shouldHaveText(text: string, locatorProps: LocatorProps = {}): Promise<void> {
        await test.step(`${this.typeOf} "${this.componentName}" should have text "${text}"`, async () => {
          const locator = this.getLocator(locatorProps);
